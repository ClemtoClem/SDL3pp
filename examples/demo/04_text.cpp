...
constexpr SDL::Point textMaxSz = windowSz / SDL::DEBUG_TEXT_FONT_CHARACTER_SIZE;
...
struct Main {
  ...
  SDL::AppResult Iterate() {
    ...
    renderer.RenderRect(SDL::FRect(cursor * SDL::DEBUG_TEXT_FONT_CHARACTER_SIZE, {SDL::DEBUG_TEXT_FONT_CHARACTER_SIZE,
                                    SDL::DEBUG_TEXT_FONT_CHARACTER_SIZE}));

    for (int i = 0; i < textMaxSz.y; i++) {
      std::string row = text.getRow(i);
      renderer.RenderDebugText( {0, float(i * SDL::DEBUG_TEXT_FONT_CHARACTER_SIZE)}, row);
    }
