...
struct FVector2 : FPoint {
	...
	[[nodiscard]] float Distance(const FVector2& o) const noexcept { return SDL::Sqrt(DistanceSq(o)); }
};
...
struct FVector3 {
	...
	/// Squared Euclidean distance to `o`.
	[[nodiscard]] constexpr float DistanceSq(const FVector3& o) const noexcept {
		return (*this - o).LengthSq();
	}

	/// Euclidean distance to `o`.
	[[nodiscard]] float Distance(const FVector3& o) const noexcept {
 ...
 */
[[nodiscard]] inline FMatrix4 ComposeTRS(const FVector3& translation,
