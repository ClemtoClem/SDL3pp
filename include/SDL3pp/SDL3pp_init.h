...
constexpr AppResult APP_EXIT_SUCCESS = SDL_APP_SUCCESS; ///< Alias for APP_SUCCESS

/// Value that requests termination with error from the main callbacks.
constexpr AppResult APP_FAILURE = SDL_APP_FAILURE;
constexpr AppResult APP_EXIT_FAILURE = SDL_APP_FAILURE; ///< Alias for APP_FAILURE
...
/// @private
template<class T>
concept HasInitFunction = requires(T** state) { { T::Init(state, AppArgs{}) } -> std::convertible_to<AppResult>;
};
