 ...
 * and later) or `2 * sizeof(void *)`, whichever is smaller. Use AlignedAlloc()
 ...
 *
 * The memory returned by this function must be freed with AlignedFree(), _not_
 * Free().
 *
 ...
 * If `mem` is nullptr, this function does nothing.
 *
 * @param mem a pointer previously returned by AlignedAlloc(), or nullptr.
 *
 * @threadsafety It is safe to call this function from any thread.
 *
 * @since This function is available since SDL 3.2.0.
 ...
 * @sa AlignedAlloc
 */
inline void AlignedFree(void* mem) { SDL_aligned_free(mem); }
 ...
 * SetenvUnsafe() or UnsetenvUnsafe() if you want changes to persist in the C
 ...
 * };
 *
 * QsortR(values, Arraysize(values), sizeof(values[0]), compare, (const void
 ...
 * data key = { 2, nullptr };
 *
 * data *result = BsearchR(&key, values, Arraysize(values), sizeof(values[0]),
...
/**
 * This works exactly like StrtokR() but doesn't require access to a C runtime.
 *
 ...
 *
 * If you want to generate a pseudo-random number in the full range of Sint32,
 * you should use: (Sint32)RandBits()
 *
 * If you want reproducible output, be sure to initialize with Srand() first.
 *
 * There are no guarantees as to the quality of the random sequence produced,
 ...
 * @sa Srand
 */
inline Uint32 RandBits() { return SDL_rand_bits(); }
 ...
 * @sa Randf
 */
inline Uint32 RandBits(Uint64* state) { return SDL_rand_bits_r(state); }
 ...
 * @sa Randf
 */
inline Uint32 RandBits(Random& state) { return state.RandBits(); }
...
/**
 * Convert radians to degrees for floating-point values.
 */
inline float RadToDeg(float radians) { return radians * 180.0f / PI_F; }

/**
 * Convert degrees to radians for floating-point values.
 */
inline float DegToRad(float degrees) { return degrees * PI_F / 180.0f; }

/**
 * Convert radians to degrees for double-precision floating-point values.
 */
inline double RadToDeg(double radians) { return radians * 180.0 / PI_D; }

/**
 * Convert degrees to radians for double-precision floating-point values.
 */
inline double DegToRad(double degrees) { return degrees * PI_D / 180.0; }
...
/**
 * Cumpute lerp between `x` and `y` by `a`.
 * 
 * @param x the start value.
 * @param y the end value.
 * @param a the interpolation factor, typically in the range [0, 1].
 ...
 * @sa IconvString
 */
inline int IconvClose(IConvRaw cd) { return CheckError(SDL_iconv_close(cd)); }

inline int IConv::close() { return IconvClose(Release()); }
 ...
 *
 * @param cd The character set conversion context, created in IconvOpen().
 * @param inbuf Address of variable that points to the first character of the
 *              input sequence.
 * @param inbytesleft The number of bytes in the input buffer.
 * @param outbuf Address of variable that points to the output buffer.
 ...
 *
 * This is a helper macro that might be more clear than calling IconvString
